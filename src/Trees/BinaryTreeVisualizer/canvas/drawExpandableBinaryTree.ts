  ...
  if (node.left) {
    const currentPathArray: PathArray =
      node.left.left || node.left.right
        ? [...pathArray, "left"]
        : [...pathArray];
    const xLeft = xPosition - 0.5 * theme.leafNodeSpacing;

    requiredRedraw =
      drawSingleNode(node.left, comp, currentPathArray, {
        x: xLeft,
        y: childHeight,
      }) || requiredRedraw;
  ...
  if (node.right) {
    const currentPathArray: PathArray =
      node.right.left || node.right.right
        ? [...pathArray, "right"]
        : [...pathArray];
    const xRight = xPosition + 0.5 * theme.leafNodeSpacing;

    requiredRedraw =
      drawSingleNode(node.right, comp, currentPathArray, {
        x: xRight,
        y: childHeight,
      }) || requiredRedraw;
    connectPointsWithBezierCurve(
...
function drawAllNodes(
  ...
  for (const currentPath of globalPathArray) {
    ...
    if (currentPath === "left") {
      ...
      xPosition = xPosition - 0.5 * theme.leafNodeSpacing;
...
function animationFrameCB(
  ...
  const { left: leftNodeWidth, right: rightNodeWidth } =
